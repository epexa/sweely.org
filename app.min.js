...
window.addEventListener('DOMContentLoaded', function() {
	...
	document.getElementById('detect-location-btn').addEventListener('click', function() {
		if (navigator.geolocation) navigator.geolocation.getCurrentPosition(function(position) {
			...
			getQuery.open('GET', 'https://maps.googleapis.com/maps/api/geocode/json?latlng=' + latitude + ',' + longitude + '&sensor=false&language=en&key=AIzaSyD8rVrtqmTpH3oR3e2XZaPxwzt3Wos6HUg', true);
	...
	$requestNewModalForm.addEventListener('submit', function(e) {
		...
		var emailVal = this.querySelector('.form-control[name="request-email"]').value,
			...
			loginTypeVal = this.querySelector('.form-control[name="request-login-type"]').value,
			loginVal = this.querySelector('.form-control[name="request-login"]').value;
		locationVal = $requestLocationInput.value;
		postAjax(emailApiUrl + '/request', {
			email: emailVal,
			acount_type: accountTypeVal,
			login_type: loginTypeVal,
			login: loginVal,
	...
	if (!localStorage.cookieAccepted) {
		document.querySelector('.cc-window').style = 'display:flex; opacity: 1; z-index: 2'
	} else {
	...
	document.querySelector('#cookie-btn').addEventListener('click', function() {
		...
		document.querySelector('.cc-window').style = 'visibility:hidden; opacity: 0; z-index: -1'
	})
	...
	var pageLang = document.documentElement.lang,
		...
		flagsPath = window.location.origin + '/graphics/flags/';

	function modalTranslate(text, yes, no) {
		document.querySelector('#text-lang').innerHTML = text;
		document.querySelector('#accept-lang').innerHTML = yes;
		document.querySelector('#dismiss-lang').innerHTML = no
	}
	if (!localStorage.lang) {
		if (browserLang !== pageLang) {
			switch (browserLang) {
				case 'en':
					pathName = '';
					modalTranslate('Your language is <img class="img-thumbnail" src="' + flagsPath + 'en.svg"><p>English?</p>', 'Yes', 'No');
					break
				case 'ru':
					pathName = '/ru';
					modalTranslate('Ваш язык <img class="img-thumbnail" src="' + flagsPath + 'ru.svg"><p>Русский?</p>', 'Да', 'Нет');
					break
				case 'it':
					pathName = '/it';
					modalTranslate('La tua lingua è <img class="img-thumbnail" src="' + flagsPath + 'it.svg"><p>Italiana?</p>', 'Sì', 'No');
					break
				case 'es':
					pathName = '/es';
					modalTranslate('Es su idioma <img class="img-thumbnail" src="' + flagsPath + 'es.svg"><p>Español?</p>', 'Si', 'No');
					break
				case 'pl':
					pathName = '/pl';
					modalTranslate('Czy twój język <img class="img-thumbnail" src="' + flagsPath + 'pl.svg"><p>jest Polski?</p>', 'Tak', 'Nie');
					break
				case 'de':
					pathName = '/de';
					modalTranslate('Ist deine Sprache <img class="img-thumbnail" src="' + flagsPath + 'de.svg"><p>Deutsch?</p>', 'Ja', 'Nein');
					break
				case 'pt':
					pathName = '/pt';
					modalTranslate('A sua língua é o <img class="img-thumbnail" src="' + flagsPath + 'pt.svg"><p>Português?</p>', 'Sim', 'Não');
					break
				case 'fr':
					pathName = '/fr';
					modalTranslate('Est-ce que votre langue est le <img class="img-thumbnail" src="' + flagsPath + 'fr.svg"><p>Français?</p>', 'Oui', 'Non');
					break
				default:
					// console.error('Can`t detect this language')
			}
			document.querySelector('#accept-lang').addEventListener('click', function() {
				window.location.pathname = pathName
			})
			localStorage.lang = browserLang;
			$('#modal-wrong-lang').modal('show')
		}
	}

	// fill beta form 15sec
	/* function betaInvitationPopup() {
		if (!localStorage.betaInvitation) setTimeout(function() {
			if ( ! localStorage.requestBeta) {
				if (document.body.classList[0] != 'modal-open') {
					$('#modal-beta-request').modal('show');
					localStorage.betaInvitation = true;
				}
				else betaInvitationPopup();
			}
		}, 15000)
	}
	...
	document.querySelector('#timer-register').addEventListener('click', function() {
		$('#modal-form').modal('show');
	...
	if (document.querySelector('#modal-feedback')) {
			var feedbackCanClose = true;
		// mouse leave site
		// https://stackoverflow.com/a/48281888
		document.addEventListener('mouseleave', function(event) {
			if (event.clientY <= 0) {
				if (event.relatedTarget == null && ! localStorage.requestBeta && ! localStorage.feedback && document.body.classList[0] != 'modal-open') {
					document.querySelector('.feedback-greeting').style = 'opacity: 1; visibility: visible; z-index: 1';
					document.querySelector('.feedback-custom-cause').style = 'opacity: 0 !important; visibility: hidden';
					feedbackCanClose = true;
					$('#modal-feedback').modal('show');
				}
			}
		});

		document.querySelector('#modal-feedback').addEventListener('mouseenter', function() {
			feedbackCanClose = false;
		});

		$('#modal-feedback').on('shown.bs.modal', function(e) {
			setTimeout(function() {
				if (feedbackCanClose) $('#modal-feedback').modal('hide');
			}, 15000);
		});
	}

	var feedbackId;
	document.querySelectorAll('.feedback-greeting .btn-send-cause').forEach(function(item) {
		item.addEventListener('click', function() {
			var causeVal = this.getAttribute('data-cause');
			postAjax(emailApiUrl + '/feedback', {
				cause: causeVal,
				lang: document.documentElement.lang
			}, function(responseJSON) {
				var responseData = JSON.parse(responseJSON);
				if (responseData.id) feedbackId = responseData.id;
				if (causeVal != 0) {
					localStorage.feedback = true;
					document.querySelector('.feedback-greeting').style = 'opacity: 0; visibility: hidden; z-index: -1';
					document.querySelector('.feedback-thanks').style = 'opacity: 1 !important; visibility: visible';
				}
				else $('#modal-feedback').modal('hide');
			});
		})
	});

	if (document.querySelector('#custom-cause')) {
		document.querySelector('#custom-cause').addEventListener('click', function() {
			document.querySelector('.feedback-greeting').style = 'opacity: 0; visibility: hidden; z-index: -1';
			document.querySelector('.feedback-custom-cause').style = 'opacity: 1 !important; visibility: visible';
			setTimeout(function() {
				document.getElementById('textarea-custom-cause').focus()
			}, 100)
		});
	}

	if (document.querySelector('#feedback-custom-cause-form')) {
		document.querySelector('#feedback-custom-cause-form').addEventListener('submit', function(e) {
			e.preventDefault();
			var customCauseVal = document.querySelector('#textarea-custom-cause').value;
			postAjax(emailApiUrl + '/feedback', {
				cause: 9,
				custom_cause: customCauseVal
			}, function(responseJSON) {
				var responseData = JSON.parse(responseJSON);
				if (responseData.id) feedbackId = responseData.id;
				localStorage.feedback = true;
				//
				document.querySelector('.feedback-custom-cause').style = 'opacity: 0; visibility: hidden; z-index: -1';
				document.querySelector('.feedback-thanks').style = 'opacity: 1 !important; visibility: visible';
				setTimeout(function() {
	...
	if (document.querySelector('#feedback-email-form')) {
		document.querySelector('#feedback-email-form').addEventListener('submit', function(e) {
			e.preventDefault();
			var emailVal = document.querySelector('#feedback-email').value;
			postAjax(emailApiUrl + '/feedbackEmail', {
				id: feedbackId,
				email: emailVal
			}, function() {
				document.querySelector('.mail-container').style = 'opacity: 0; visibility: hidden; transition: .9s';
				document.querySelector('.okay-container').style = 'opacity: 1 !important; visibility: visible; transition: .9s';
			});
		});
	}
